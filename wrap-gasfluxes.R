...
wrap_gasfluxes <- function(gc.samples, gas.species, f.detect){
  ...
  # =>> use this!!!  gasfluxes(hmr.input,.id = c("ID","block"))
  ...
  gc.samples$shiny.gas   <-  gas.concentration * gas.molarweight    * 1E-6    ## mg-N2O / m2 / h    DiverFarming unit suggested by Kristiina Regina 
  gc.samples[, c("N2O.ECD.mol","CO2.TCD.mol","CH4.FID.mol") := NULL]
  gc.samples[,vial.time := vial.time / 3600]
  # gc.samples[,chamber.time := tstrsplit(chamber.time,":")[[1]]]
  ...
  gc.samples  <- na.omit(gc.samples)
  gasflux.output <- gasfluxes(gc.samples, .id = c("date","site","chamber.nr","treatment_fact1","treatment_fact2", # "chamber.time",
                                                  "treatment_fact3","block","sub_factor"), .V = "chamber.V", .A = "chamber.A",
                              .times = "vial.time",  .C = "shiny.gas", methods = c("linear", "robust linear", "HMR")
                              ,k_HMR= log(1.5),plot = T)   # use gasflux package by Roland to actually calculate the fluxes with different models
  ...
  gasflux.output$f.detect <- f.detect
  gasflux.output$t.meas  <- 1 # gc.samples[, .(t.meas = max(vial.time)), by = sample.nr]$t.meas
  ...
  #                                f.detect.co2 = c(0.01,  0.003, 0.003 ),  # unit of CO2 different to the others
  ...
  gasflux.select <- selectfluxes(gasflux.output, select = "kappa.max", f.detect = gasflux.output$f.detect, t.meas = gasflux.output$t.meas)
  setnames(gasflux.select,"flux","dynamic.kappa.f0") # rename output flux column to kappa max dynamic selection
  # setnames(gasflux.select,"method","method.selected") # rename output 
  ...
  # gasflux.select$date <- format.Date(strptime(gasflux.select$date, format = "%d-%m-%y"),format="%Y-%m-%d")
  ...
  write.table(gasflux.select, paste("data/gasflux-",gas.species,"webinput.csv",sep = ""), sep=";", dec=".",row.names=F,col.names=T, append=F)   # writes data from GC extration to output file
